<?php
	...
	$data = mysqli_query($koneksi,"select * from t_pengeluaran WHERE kode_pengeluaran='$id'");				
	while ($d = mysqli_fetch_assoc($data)){
		$kodee = $d['KODE_PENGELUARAN'];
		$notess = $d['NOTES'];
		$kategg = $d['KATEGORI'];
		$sourcee =$d['SOURCE'];
		$nomin = $d['NOMINAL'];
		$nomin_tamp = "Rp".number_format($nomin,0,",",".");	
		$semua = "Kode: ".$kodee." - Keterangan: ".$notess." - Keperluan: ".$kategg." - Source: ".$sourcee." - Nominal: ".$nomin_tamp;		
	}
	$notes = $oleh." - HAPUS - ".$id;
	$tanda = 'HAPUS PENGELUARAN';
	...
	if($_SESSION['status']!="login"){
		...
		$query="DELETE from t_pengeluaran where KODE_PENGELUARAN='$id'";
		if (mysqli_query($koneksi, $query)) {
				if (mysqli_query($koneksi, $query_log)) {
					echo "<script>alert('Data terhapus');window.location.href='data-pengeluaran';</script>";		
				}				
			} else {
