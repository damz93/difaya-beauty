...
<html moznomarginboxes mozdisallowselectionprint>
	<head>
		<head>
		<title>Laporan Admin - DIFAYA BEAUTY | Effortless Care For Priceless You</title>
	...
	<body>
		<?php 
			...
			if (isset($_POST['tampil_tgl_admin'])){			
				...
				if((strcmp($tglawal,$tglakhir)==0) OR empty($tglakhir)){
					...
					$nama = "Laporan Summary Admin ( " . formatTanggal($tglawal) ." s.d. ". formatTanggal($tglakhir)." )";
				...
				if((strcmp($bulawal,$bulakhir)==0) OR empty($bulakhir)){
					$nama = "Laporan Summary Admin Bulanan - " . $bulawal;
					...
					//$data_list = mysqli_query($koneksi, "SELECT * FROM t_pemasukan WHERE SOURCE='".$sourcex."' AND MONTH(TGL)=".$bulanaw." AND YEAR(TGL)=".$tahunaw." GROUP BY TGL ORDER BY TGL DESC");
					...
					$nama = "Laporan Summary Admin ( " . $bulawal ." s.d. ". $bulakhir." )";
			...
			else if (isset($_POST['tampil_semua_admin'])){	
				$nama = "Laporan Semua Summary Admin";
			...
			$kas = $keuntungan/2;
			$kas_tamp = number_format($kas,0,',','.');	
			$bersih = $keuntungan/2;
			$bersih_tamp = number_format($bersih,0,',','.');				
			$admin = $bersih*0.1;
			$admin_tamp = number_format($admin,0,',','.');	
			
			?>
			
		<u>
			<h1 class="center">SUMMARY ADMIN</h1>
		</u>
		...
		<table border="0" style="width:25%" align='center'>
			<tr>
				...
				<td align="right" width="12%">
					<h6><?php echo "Rp".$kas_tamp ?></h6>
					<h6><?php echo "Rp".$bersih_tamp ?></h6>
					<h6><?php echo "Rp".$admin_tamp ?></h6>
				</td>				-->
			</tr>
		</div>
