<?php
	...
	$data = mysqli_query($koneksi,"select * from t_pemasukan WHERE KODE_PEMASUKAN='$kode_pemasukan'");				
	while ($d = mysqli_fetch_assoc($data)){
		$kodee2 = $d['KODE_PEMASUKAN'];
		$notess2 = $d['NOTES'];
		$kategg2 = $d['KATEGORI'];
		$sourcee2 =$d['SOURCE'];
		$nomin2 = $d['NOMINAL'];
		...
		if($kategori!=$kategg2){
			$semua = 'Kategori: '.$kategg2.' -> '.$kategori;
		}
		if($notes!=$notess2){
			if ($semua!=''){
				$semua = $semua." - ";
	...
	$notes2 = $oleh." - EDIT - ".$kode_pemasukan;
	$tanda2 = 'EDIT PEMASUKAN';
	$semua2 = $semua;
	...
	if ($semua2!=''){		
		$query_log=mysqli_query($koneksi,"INSERT INTO t_log(KODE_LOG,TANDA,NOTES,ISI,`SOURCE`,TGL,WAKTU,OLEH,KETERANGAN)VALUES('$kode_log','$tanda2','$notes2','$semua2','$sumberrrr','$tgl_saja','$waktu_skg','$oleh','$ketee')");
			...
			if ($_FILES['upd_foto']['size'] != 0){
				...
				if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
					if($ukuran < 1044070){			
						//move_uploaded_file($file_tmp, 'img-pemasukan/'.$nama);
						$nama_baru = $kode_pemasukan.".".$ekstensi;
						move_uploaded_file($file_tmp, 'img-pemasukan/'.$nama_baru);
						$query=mysqli_query($koneksi,"UPDATE t_pemasukan SET FOTO='$nama_baru',SOURCE='$sumber',KATEGORI='$kategori',NOTES='$notes',NOMINAL='$nominal_simp',WAKTU='$waktu_skg',OLEH='$oleh',KETERANGAN='$ketee' WHERE KODE_PEMASUKAN='$kode_pemasukan'");							
						if($query){	
							if($query_log){						
								echo "<script>alert('Data Terupdate...');window.location.href='data-pemasukan';</script>";						
							}							
						}else{						
				...
				$query=mysqli_query($koneksi,"UPDATE t_pemasukan SET SOURCE='$sumber',KATEGORI='$kategori',NOTES='$notes',NOMINAL='$nominal_simp',WAKTU='$waktu_skg',OLEH='$oleh',KETERANGAN='$ketee' WHERE KODE_PEMASUKAN='$kode_pemasukan'");						
				if($query){	
					if($query_log){						
						echo "<script>alert('Data Terupdate...');window.location.href='data-pemasukan';</script>";						
					}
		...
		echo "<script>alert('Tidak ada data yang diubah...');window.location.href='data-pemasukan';</script>";	
	}
?>